...
{
	mat4_identity(out);
	mat4_rotate(out, (float[3]){1, 0, 0}, m.rot[0]);
	mat4_rotate(out, (float[3]){0, 1, 0}, m.rot[1]);
	mat4_rotate(out, (float[3]){0, 0, 1}, m.rot[2]);
	mat4_translate(out, m.pos);
}
